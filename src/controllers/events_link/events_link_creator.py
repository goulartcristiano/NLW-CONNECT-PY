...
class EventsLinkCreator:
    ...
    def create(self, http_request: HttpRequest) -> HttpResponse:
        ...
        subscriver_id = event_link_info["subscriber_id"]

        self.__check_event_link(event_id, subscriver_id)
        new_link = self.__create_event_link(event_id, subscriver_id)
        return self.__format_response(new_link, event_id, subscriver_id)
    ...
    def __check_event_link(self, event_id: int, subscriber_id: int) -> None:
        ...
        if response: raise Exception("Link já existe!")
